import { ADDITIONAL_RECIPES } from "./recipeCatalogue.js";
...
export const INGREDIENTS = {
  ...
  chickpeas: { name: "Kichererbsen (gegart, abgetropft)", match: /^(kichererbsen)$/i, requiresCooked: true },
  kidney: { name: "Kidneybohnen (gegart, abgetropft)", match: /^(kidneybohnen|kidney beans)$/i, requiresCooked: true },
  whiteBeans: { name: "Weiße Bohnen (gegart, abgetropft)", match: /^(weiße bohnen|weisse bohnen)$/i, requiresCooked: true },
  corn: { name: "Mais (abgetropft)", match: /^(mais|gemüsemais|gemuesemais|zuckermais)$/i, requiresCooked: true },
  passata: { name: "Passierte Tomaten", match: /^(passierte tomaten|tomaten passiert|passata)$/i },
  pepper: { name: "Paprika", match: /^(paprika|paprika rot|paprika gelb|paprika mix|spitzpaprika)$/i },
  coconut: { name: "Kokosmilch", match: /^(kokosmilch)$/i },
  curry: { name: "Currypulver", match: /^(currypulver|curry pulver)$/i },
  paprika: { name: "Paprikapulver edelsüß", match: /^(paprikapulver edelsüß|paprika edelsüß)$/i },
  couscous: { name: "Couscous", match: /^(couscous|cous cous)$/i },
  bulgur: { name: "Bulgur", match: /^(bulgur)$/i },
  cucumber: { name: "Gurke", match: /^(gurke|gurken|salatgurke|salatgurken)$/i },
  lemon: { name: "Zitronensaft", match: /^(zitronensaft)$/i },
  mushroom: { name: "Champignons", match: /^(champignons|champignons braun|champignons weiß|braune champignons|weiße champignons)$/i },
  creamCheese: { name: "Frischkäse natur", match: /^(frischkäse natur|frischkaese natur)$/i },
  aubergine: { name: "Aubergine", match: /^(aubergine|auberginen)$/i },
  garlic: { name: "Knoblauch", match: /^(knoblauch)$/i },
  peas: { name: "Erbsen (tiefgekühlt)", match: /^(erbsen tiefgekühlt|tiefkühlerbsen|tk erbsen)$/i },
  gnocchi: { name: "Gnocchi", match: /^(gnocchi|kartoffelgnocchi)$/i },
  mozzarella: { name: "Mozzarella", match: /^(mozzarella)$/i },
  pumpkin: { name: "Hokkaido-Kürbis", match: /^(hokkaido|hokkaido kürbis|hokkaido-kürbis)$/i },
  leek: { name: "Lauch", match: /^(lauch|porree)$/i },
  cauliflower: { name: "Blumenkohl", match: /^(blumenkohl)$/i },
  sweetPotato: { name: "Süßkartoffeln", match: /^(süßkartoffeln|süsskartoffeln)$/i },
  soySauce: { name: "Sojasauce", match: /^(sojasauce|sojasoße)$/i },
  cabbage: { name: "Spitzkohl", match: /^(spitzkohl)$/i },
};
...
// Canned-legume recipes must never match a dry bag by name alone.
export function matchesIngredient(offer, ingredientId) {
  const definition = INGREDIENTS[ingredientId];
  const name = (offer.productName || '').trim();
  if (!definition || !definition.match.test(name)) return false;
  if (!definition.requiresCooked) return true;
  const description = offer.description || '';
  return !/getrocknet|ungekocht|trocken/i.test(description) && /dose|konserve|glas|gegart|gekocht|abgetropft|abtropfgewicht/i.test(description);
}
export function recommendRecipes(offers, prefs, day) {
  ...
  return RECIPES.filter(r => (prefs.diet !== 'vegan' || r.diet === 'vegan') && r.minutes <= prefs.minutes && (!prefs.mealPrep || r.mealPrep) && !r.ingredients.some(i => excluded.has(i.id))).map(r => {
    const ingredients = r.ingredients.map(i => {
      ...
      const matches = eligible.filter(o => matchesIngredient(o, i.id));
      const priced = matches.map(offer => { const pack = INGREDIENTS[i.id].requiresCooked ? null : packAmount(offer,i.unit); return { offer, pack, checkout: pack ? Math.ceil(amount / pack) * offer.price : null }; });
