...
const API_TANKERKOENIG_LIST = "https://creativecommons.tankerkoenig.de/json/list.php";
const LEAFLET_IMG_CDN = "https://mg2de.b-cdn.net/api/v1/leaflets";

const requestCache = new Map();

async function withCache(key, load, { force = false } = {}) {
  const cached = requestCache.get(key);
  if (!force && cached && Date.now() - cached.createdAt < 15 * 60 * 1000) return cached.pending;

  const pending = (async () => {
    try {
      return await load();
    } catch (error) {
      if (requestCache.get(key)?.pending === pending) requestCache.delete(key);
      throw error;
    }
  })();

  requestCache.set(key, { pending, createdAt: Date.now() });
  return pending;
}
...
export async function fetchRetailerOffers(
  retailerSlug,
  zip,
  {
    limit = 500,
  ...
  const key = ["retailer-offers", retailerSlug, zip, limit, offset].join(":");
  return withCache(
    key,
    async () => {
      ...
      try {
        ...
        throw new Error(`Netzwerkfehler für ${retailerSlug}: ${e.message}`);
      }
...
export async function fetchPublishers(zip, { limit = 50, offerLimit = 500, showAllIndustries = false, force = false } = {}) {
  const key = ["publishers", zip, limit, offerLimit, showAllIndustries ? 1 : 0].join(":");
  return withCache(
    key,
    async () => {
      ...
      try {
        ...
        console.error("[SP] net err:", e);
        throw new Error(`Netzwerkfehler: ${e.message}`);
      }
      if (!resp.ok) {
        const b = await resp.text().catch(() => "");
        console.error(`[SP] ${resp.status}:`, b.slice(0, 200));
        throw new Error(`API Fehler ${resp.status}`);
      }
      const data = await resp.json();

      const groupsRaw = await Promise.all(
        (data.results || []).map(async (pub) => {
          let offers = [];
          const fallbackIndustryIds = Array.from(
            new Set(
              (pub.offers || [])
                .flatMap((o) => (o.industries || []).map((i) => i.id))
          ...
          const fallbackCategoryNames = Array.from(
            new Set(
              (pub.offers || [])
                .flatMap((o) => (o.categories || []).map((c) => c.name))
                .filter(Boolean)
            )
          );
          try {
      ...
      console.log(`[SP] ${groups.length} grocery retailers, ${groups.reduce((sum, group) => sum + group.offers.length, 0)} offers`);
      return groups;
    },
    { force }
  );
...
export async function apiSearch(query, zip, limit = 200, { force = false } = {}) {
  const key = ["search", zip, limit, query.trim().toLowerCase()].join(":");
  return withCache(
    key,
    async () => {
...
export async function fetchLeafletFlights(zip, { showAllIndustries = false, force = false } = {}) {
  const key = ["leaflet-flights", zip, showAllIndustries ? 1 : 0].join(":");
  return withCache(
    key,
    async () => {
...
export async function fetchLeafletDetail(leafletId, zip, { force = false } = {}) {
  const key = ["leaflet-detail", leafletId, zip].join(":");
  return withCache(
    key,
    async () => {
...
export async function fetchLeafletPageOffers(leafletId, pageIndex, { force = false } = {}) {
  const key = ["leaflet-page-offers", leafletId, pageIndex].join(":");
  return withCache(
    key,
    async () => {
...
export async function fetchBestLeaflet(flightId, zip, coords, { force = false } = {}) {
  const key = [
    "best-leaflet",
    flightId,
    zip,
...
export async function geocodeZip(zip, { force = false } = {}) {
  if (!zip || zip.length < 5) return null;

  const key = ["geocode", zip].join(":");
  return withCache(
    key,
    async () => {
      try {
...
export async function fetchStoreLocations(leafletFlights, zip, coords, { force = false } = {}) {
  const entries = Object.entries(leafletFlights);
  const flightsKey = entries
    .map(([slug, flights]) => `${slug}:${flights[0]?.id || "none"}`)
    .sort()
    .join("|");
  const key = ["store-locations", zip, coords?.lat ?? "no-lat", coords?.lon ?? "no-lon", flightsKey].join(":");
    ...
    async () => {
      ...
      await Promise.allSettled(
        entries.map(async ([slug, flights]) => {
          ...
          const best = await fetchBestLeaflet(flights[0].id, zip, coords, { force });
...
export async function fetchFuelStations({ zip, radius = 5, apiKey, coords, force = false } = {}) {
  if (!zip || zip.length < 5) throw new Error("Bitte eine gueltige Postleitzahl eingeben.");
  if (!apiKey?.trim()) throw new Error("Tankerkönig API-Key fehlt.");

  const resolvedCoords = coords || await geocodeZip(zip, { force });
  if (!resolvedCoords?.lat || !resolvedCoords?.lon) {
    throw new Error(`Koordinaten fuer PLZ ${zip} konnten nicht aufgeloest werden.`);
  }

  const radiusKm = Math.min(25, Math.max(1, Number.parseFloat(radius) || 5));
  const key = [
    "fuel-stations",
    zip,
    radiusKm,
    resolvedCoords.lat.toFixed(5),
  ...
  return withCache(
    ...
    async () => {
      const params = new URLSearchParams({
        ...
        lng: String(resolvedCoords.lon),
        rad: String(radiusKm),
        sort: "dist",
        type: "all",
        apikey: apiKey.trim(),
      });
      const url = `${API_TANKERKOENIG_LIST}?${params.toString()}`;
      console.log("[SP] fuel stations:", `${API_TANKERKOENIG_LIST}?lat=${resolvedCoords.lat}&lng=${resolvedCoords.lon}&rad=${radiusKm}&sort=dist&type=all&apikey=***`);
      ...
      if (!resp.ok) throw new Error(`Tankerkönig API Fehler ${resp.status}`);
      ...
      return {
        ...
        stations: (data.stations || []).map((station) => ({
          ...
          postCode: station.postCode ? String(station.postCode) : "",
          place: station.place || "",
