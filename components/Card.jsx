...
export function Card({o, sm, showRetailer=false, highlightQuery="", added, addItem, myLoyalty=[]}) {
  ...
  const myL=isMyLoyalty(o.retailerSlug,o.retailerName,myLoyalty);
      ...
      <div style={{textAlign:"right",flexShrink:0,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:"5px",paddingTop:d>0?"12px":0}}>
        <div>
          <div style={{display:"flex",alignItems:"baseline",gap:"3px",justifyContent:"flex-end"}}>
            ...
            <span style={{fontSize:sm?"17px":"19px",fontWeight:800,fontFamily:"'JetBrains Mono',monospace",lineHeight:1}}>{o.price.toFixed(2)}</span>
            <span style={{fontSize:"10px",color:"#999"}}>€</span>
          </div>
          {up!==null&&<div style={{fontSize:"10px",color:"#059669",fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{up.toFixed(2)}€/{o.unitShort||"kg"}</div>}
        </div>
