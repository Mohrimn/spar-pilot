  ...
  return (
    <div>
      <form onSubmit={doSearch} style={{ padding: "12px 18px 4px", position: "sticky", top: 0, zIndex: 50, background: "#f5f4f0" }}>
        ...
        <div style={{ fontSize: "10px", color: "#999", fontFamily: "'DM Mono',monospace", padding: "6px 2px 0" }}>Globale Suche über alle Händler</div>
        {sDone && sRes.length > 0 && <div style={{ display: "flex", gap: "4px", marginTop: "8px", alignItems: "center", flexWrap: "wrap", paddingBottom: "4px" }}>
          ...
          <span style={{ fontSize: "10px", color: "#bbb", fontFamily: "'DM Mono',monospace", marginLeft: "auto" }}>{fs.length}</span>
          ...
          <span style={{ fontSize: "10px", color: "#bbb", fontFamily: "'DM Mono',monospace", width: "100%" }}>
      ...
      <div style={{ padding: "6px 18px 18px" }}>
        ...
        {!loading && !sDone && !err && <div style={{ textAlign: "center", padding: "48px 0", color: "#bbb" }}><div style={{ fontSize: "32px", marginBottom: "8px" }}>🔍</div><div style={{ fontSize: "13px", fontWeight: 500 }}>Produkt suchen</div><div style={{ fontSize: "11px", marginTop: "3px", fontFamily: "'DM Mono',monospace" }}>"milch OR hafermilch" · kell* · "irische butter"</div></div>}
        ...
        {sDone && !loading && sRes.length > 0 && fs.length === 0 && !err && <div style={{ textAlign: "center", padding: "48px 0", color: "#bbb" }}><div style={{ fontSize: "32px", marginBottom: "8px" }}>⏳</div><div style={{ fontSize: "13px" }}>Nur kommende Angebote gefunden</div><div style={{ fontSize: "11px", marginTop: "3px", fontFamily: "'DM Mono',monospace" }}>Filter „Nur gestartet" deaktivieren</div></div>}
        ...
        {searchGroupBy === "none" && fs.map(o => <div key={o.id} style={{ marginBottom: "6px", animation: "fadeIn 0.2s ease" }}><Card o={o} added={added} addItem={addItem} /></div>)}
        {searchGroupBy !== "none" && searchGrouped.map(g => { const vendorMode = searchGroupBy === "vendor"; const rc = vendorMode ? getRetColor(g.offers[0]?.retailerSlug, g.name) : "#1a1a1a"; const lt = isLightColor(rc); return <div key={g.name} style={{ marginBottom: "10px", animation: "fadeIn 0.2s ease" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 10px", background: vendorMode ? rc : "#fff", border: vendorMode ? "none" : "1px solid #eee", borderRadius: "9px 9px 4px 4px", color: vendorMode ? (lt ? "#1a1a1a" : "#fff") : "#1a1a1a" }}>
            ...
            <span style={{ fontSize: "10px", fontFamily: "'DM Mono',monospace", opacity: vendorMode ? 0.8 : 0.6 }}>{g.offers.length}</span>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "5px", paddingTop: "5px" }}>
            {g.offers.map(o => <Card key={o.id} o={o} sm showRetailer={searchGroupBy === "category"} added={added} addItem={addItem} />)}
          </div>
        </div> })}
      </div>
    </div>
